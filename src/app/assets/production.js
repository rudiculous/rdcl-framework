...
exports.construct = function* construct(app, baseDir) {
    ...
    const mediaBaseUrl = app.mediaBaseUrl;

    app.locals.add_javascript_tag = function addJavaScriptTag(file) {
        if (manifest[file] != null) {
            return helpers.tags.html`<script type="text/javascript" src="${mediaBaseUrl}assets/${manifest[file]}"></script>\n`;
        }
        else {
            return '';
        }
    ...
    app.locals.add_stylesheet_link = function addStylesheetLink(file) {
        if (manifest[file] != null) {
            return helpers.tags.html`<link rel="stylesheet" type="text/css" href="${mediaBaseUrl}assets/${manifest[file]}"></script>\n`;
        }
        else {
            return '';
        }
